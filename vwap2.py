...
def calc_order_quantity(raw_order_qty, round_lot, qty_remaining):
    ...
    if raw_order_qty >= round_lot:  # round to nearest lot
        return np.floor(raw_order_qty/round_lot)*round_lot
...
# MAIN ALGO LOOP
def algo_loop(trading_day, order_side, original_order_quantity, vwap_coefficients, last_min_penalty = 1., round_lot =100., schedule_coef=0., tick_coef=0., model_coef=0.,using_model=False):
    ...
    for index, row in trading_day.iterrows():
        ...
        fair_value = midpoint + (schedule_coef * schedule_factor * half_spread) + (
                tick_coef * tick_factor * half_spread) + (model_coef * model_factor * half_spread)
        # collect our data
        ...
        if order_side == 'b':
            if (fair_value >= ask_price or current_bar == last_minutes_from_open) and quantity_behind > round_lot:
                if current_bar == last_minutes_from_open:
                    round_lot = 1
                    tmp = pd.DataFrame(trades, columns=['price', 'shares', 'bar', 'trade_type'])
                    print(tmp.shape)
                    print(tmp.shares.sum())
                    print(tmp.tail())
                    quantity_behind = original_order_quantity - tmp.shares.sum()
                    last_min_order = quantity_behind
                ...
                if current_bar == last_minutes_from_open:
                    new_trade_price += last_min_penalty*(last_min_order/original_order_quantity)*half_spread*2
                ...
                # To update quantity_behind and eliminate bug.
                quantity_behind = current_target_shares - total_quantity_filled
    ...
    trades = pd.DataFrame(trades, columns=['price', 'shares', 'bar', 'trade_type'])
