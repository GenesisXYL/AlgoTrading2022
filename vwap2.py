...
# MAIN ALGO LOOP
def algo_loop(trading_day, order_side, original_order_quantity, vwap_coefficients, schedule_coef, tick_coef,model_coef=0,using_model=False):
        ...
        if order_side == 'b':
            if (fair_value >= ask_price or current_bar == last_minutes_from_open) and quantity_behind > round_lot:
                if current_bar == last_minutes_from_open:
                    round_lot = 1
                    quantity_behind = original_order_quantity - trades['shares'].sum()
